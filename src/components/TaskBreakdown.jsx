...
import ExpectationsTracker from './ExpectationsTracker'
import TaskMetadataEditor from './TaskMetadataEditor'
import { EFFORT_LEVELS, SKILL_LEVELS } from '../utils/goalAnalyzer'
import { processExpectationsTracker, updateTaskMetadata, loadTaskMetadata } from '../utils/expectationsTracker'
...
function TaskBreakdown({ goal, breakdown, onReset, onOpenHistory, onRegenerate, isRegenerating }) {
  ...
  const [refreshKey, setRefreshKey] = useState(0) // Force refresh of expectations tracker
  ...
  useEffect(() => {
    if (goal) {
      ...
      // Trigger expectations tracker refresh when completion state changes
      setRefreshKey(prev => prev + 1)
    }
  }, [completionState, goal])

  // Auto-refresh expectations tracker every 5 minutes
  useEffect(() => {
    if (!goal || !breakdown) return
  ...
  const toggleTaskCompletion = (taskId) => {
    ...
    setCompletionState(prev => {
      ...
      // Update lastUpdated timestamp when completing/uncompleting
  ...
  const toggleSubtaskCompletion = (taskId, subtaskIndex) => {
    setCompletionState(prev => {
      ...
      // Update lastUpdated timestamp when subtask is toggled
      updateTaskMetadata(goal, taskId, {
        lastUpdated: new Date().toISOString()
      })
  ...
  // Process expectations tracker data
  ...
  // Create a map of taskId to tracker task data for easy lookup
  const trackerTaskMap = useMemo(() => {
    if (!trackerData?.tasks) return {}
    const map = {}
  ...
  const handleTaskMetadataUpdate = (taskId, updates) => {
    updateTaskMetadata(goal, taskId, updates)
    setRefreshKey(prev => prev + 1) // Trigger refresh
    setEditingTask(null)
  }

  const handleEditTaskMetadata = (task) => {
    setEditingTask(task)
  }
  ...
  return (
    <div className="task-breakdown-container">
      ...
      {selectedView === 'tasks' ? (
        <div className="tasks-container">
          {breakdown.tasks.map((task) => {
            const trackerTask = trackerTaskMap[task.id]
            return (
              <TaskCard
                key={task.id}
                task={task}
                ...
                riskLevel={trackerTask?.riskLevel}
                assignee={trackerTask?.assignee}
                dueDate={trackerTask?.dueDate}
                daysUntilDue={trackerTask?.daysUntilDue}
                status={trackerTask?.status}
                onEditMetadata={handleEditTaskMetadata}
              />
            )
          })}
        </div>
      ...
      {editingTask && (
        <TaskMetadataEditor
          task={editingTask}
          taskMetadata={loadTaskMetadata(goal)[editingTask.id] || {}}
          onSave={(updates) => handleTaskMetadataUpdate(editingTask.id, updates)}
