      ...
      <div className="products-container">
        ...
        <div className="grid">
          {displayed.map((product) => {
            const category = Object.keys(watches).find(cat => 
              watches[cat].some(w => w.id === product.id)
            );
            return (
              <div key={product.id} className="product-card" onClick={() => setSelectedProduct(product)}>
                <div className="image-wrapper">
                  {category && <span className="category-badge">{category.charAt(0).toUpperCase() + category.slice(1)}</span>}
